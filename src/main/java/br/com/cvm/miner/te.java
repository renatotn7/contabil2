...
public static void main(String[] args) {
	 ...
	 Query queryE = em.createQuery("SELECT b FROM Empresa b where  exists ( select 1 from Demonstrativo c where  b.cvm = c.empresa.cvm)");
	  ...
	  //  StringBuilder sb = new StringBuilder();
		...
		if(dem.getIdDemonstrativo()==354) {
			System.out.println("aqui");
			 ...
			 Query query = em.createQuery("SELECT b,c FROM ValorContabil b,Calculo c where b.contaContabil.idCalculo = c.idCalculo and c.indicador.idIndicador = "+idindicador+" and  c.preferencia = 1 and b.demonstrativo.idDemonstrativo = "+dem.getIdDemonstrativo()+" order by  c.posicao");
			...
			c = (Calculo)valores.get(0)[1];
				 ...
				 temerro=true;
				 sberro.append("nada encontrado no demonstrativo:  "+dem.getIdDemonstrativo() + " cvm: " + dem.getEmpresa().getCvm() + " ativo "+ dem.getEmpresa().getRaizAtivo() +" data: "+ dem.getData() +"\n");
				 continue;
			 }
			 String expressao=c.getExpressao().getExpressao();
			int numcolumns= c.getExpressao().getNumColunas();
			 if(valores.size()!=numcolumns) {
				 //problema: não encontrou o numero necessário de colunas para este demonstrativo
				 for(int i = 1 ; i <= numcolumns;i++) {
					try {
					 Calculo c1 = (Calculo)valores.get(i-1)[1];
					 if(c1.getPosicao()!=i) {
						 temerro=true;
						 sberro.append("não encontrou coluna: "+i+ " demonstrativo:  "+dem.getIdDemonstrativo() + " cvm: " + dem.getEmpresa().getCvm() + " ativo "+ dem.getEmpresa().getRaizAtivo() +" data: "+ dem.getData()+"\n" );
					 }
					}catch(Exception e) {
				 ...
				 if(!temerro) {
					 sb.append("não encontrou erro: demonstrativo:  "+dem.getIdDemonstrativo() + " cvm: " + dem.getEmpresa().getCvm() + " ativo "+ dem.getEmpresa().getRaizAtivo() +" data: "+ dem.getData()+"\n" );
