...
return this},relayout:function relayout(){this._layout();return this},_removeSliderEventHandlers:function _removeSliderEventHandlers(){this.handle1.removeEventListener("keydown",this.handle1Keydown,!1);this.handle2.removeEventListener("keydown",this.handle2Keydown,!1);if(this.showTooltip){this.handle1.removeEventListener("focus",this.showTooltip,!1);this.handle2.removeEventListener("focus",this.showTooltip,!1)}
...
this.sliderElem.removeEventListener("touchstart",this.mousedown,!1);this.sliderElem.removeEventListener("mousedown",this.mousedown,!1);window.removeEventListener("resize",this.resize,!1)},_bindNonQueryEventHandler:function _bindNonQueryEventHandler(evt,callback){if(this.eventToCallbackMap[evt]===undefined){this.eventToCallbackMap[evt]=[]}
this.eventToCallbackMap[evt].push(callback)},_unbindNonQueryEventHandler:function _unbindNonQueryEventHandler(evt,callback){var callbacks=this.eventToCallbackMap[evt];if(callbacks!==undefined){for(var i=0;i<callbacks.length;i++){if(callbacks[i]===callback){callbacks.splice(i,1);break}}}},_cleanUpEventCallbacksMap:function _cleanUpEventCallbacksMap(){var eventNames=Object.keys(this.eventToCallbackMap);for(var i=0;i<eventNames.length;i++){var eventName=eventNames[i];this.eventToCallbackMap[eventName]=null}},_showTooltip:function _showTooltip(){if(this.options.tooltip_split===!1){this._addClass(this.tooltip,'in');this.tooltip_min.style.display='none';this.tooltip_max.style.display='none'}else{this._addClass(this.tooltip_min,'in');this._addClass(this.tooltip_max,'in');this.tooltip.style.display='none'}
...
this._state.over=!1},_layout:function _layout(){var positionPercentages;if(this.options.reversed){positionPercentages=[100-this._state.percentage[0],this.options.range?100-this._state.percentage[1]:this._state.percentage[1]]}else{positionPercentages=[this._state.percentage[0],this._state.percentage[1]]}
...
return!0},_triggerFocusOnHandle:function _triggerFocusOnHandle(handleIdx){if(handleIdx===0){this.handle1.focus()}
...
var percentage=this._getPercentage(ev);this._adjustPercentageForRangeSliders(percentage);this._state.percentage[this._state.dragged]=percentage;this._layout();var val=this._calculateValue(!0);this.setValue(val,!0,!0);return!1},_adjustPercentageForRangeSliders:function _adjustPercentageForRangeSliders(percentage){if(this.options.range){var precision=this._getNumDigitsAfterDecimalPlace(percentage);precision=precision?precision-1:0;var percentageWithAdjustedPrecision=this._applyToFixedAndParseFloat(percentage,precision);if(this._state.dragged===0&&this._applyToFixedAndParseFloat(this._state.percentage[1],precision)<percentageWithAdjustedPrecision){this._state.percentage[0]=this._state.percentage[1];this._state.dragged=1}else if(this._state.dragged===1&&this._applyToFixedAndParseFloat(this._state.percentage[0],precision)>percentageWithAdjustedPrecision){this._state.percentage[1]=this._state.percentage[0];this._state.dragged=0}}},_mouseup:function _mouseup(){if(!this._state.enabled){return!1}
...
return offsetTop},_offset:function _offset(obj){return{left:this._offsetLeft(obj),top:this._offsetTop(obj)}},_css:function _css(elementRef,styleName,value){if($){$.style(elementRef,styleName,value)}else{var style=styleName.replace(/^-ms-/,"ms-").replace(/-([\da-z])/gi,function(all,letter){return letter.toUpperCase()});elementRef.style[style]=value}},_toValue:function _toValue(percentage){return this.options.scale.toValue.apply(this,[percentage])},_toPercentage:function _toPercentage(value){return this.options.scale.toPercentage.apply(this,[value])},_setTooltipPosition:function _setTooltipPosition(){var tooltips=[this.tooltip,this.tooltip_min,this.tooltip_max];if(this.options.orientation==='vertical'){var tooltipPos=this.options.tooltip_position||'right';var oppositeSide=tooltipPos==='left'?'right':'left';tooltips.forEach((function(tooltip){this._addClass(tooltip,tooltipPos);tooltip.style[oppositeSide]='100%'}).bind(this))}else if(this.options.tooltip_position==='bottom'){tooltips.forEach((function(tooltip){this._addClass(tooltip,'bottom');tooltip.style.top=22+'px'}).bind(this))}else{tooltips.forEach((function(tooltip){this._addClass(tooltip,'top');tooltip.style.top=-this.tooltip.outerHeight-14+'px'}).bind(this))}}};if($){var namespace=$.fn.slider?'bootstrapSlider':'slider';$.bridget(namespace,Slider)}})($);return Slider});$(document).ready(function(){var main_menu_dd_size;$.each($(".nav-main-menu .dropdown"),function(){main_menu_dd_size=$(this).width();$(this).children(".dropdown-menu").width(main_menu_dd_size+40)})
$("#search-box").focus(function(){$(this).parent().addClass("search-focused");$(".nav-main-menu .navbar-item").css("display","none")})
$("#search-box").focusout(function(){$(this).parent().removeClass("search-focused");$(".nav-main-menu .navbar-item").css("display","table-cell")})
var nav_inst_inner=$("#nav-institucional").html();$("#nav-institucional-inner ul").append(nav_inst_inner);var nav_menu_items=$(".nav-anchor").children(".content").children(".section-all-items").html();$(".nav-anchor .content .navbar-item .dropdown-menu").append(nav_menu_items);$(".nav-anchor li.section-items a").click(function(){$(this).parent().parent().find(".active").removeClass("active");$(this).addClass("active")})
navAnchorBehaviour();setBannerHeight();$(".nav-abas a").click(function(){$(this).parent().children(".selected").removeClass("selected");$(this).addClass("selected")})
$.each($(".nav-abas"),function(){$.each($(".nav-abas a"),function(i){$(this).attr("id","nav-btn-"+(i+1));$("#nav-btn-"+(i+1)).click(function(){$(".nav-abas-cnt div").hide();$("#nav-ctn-"+(i+1)).show()})})
$.each($(".nav-abas-cnt div"),function(i){$(this).attr("id","nav-ctn-"+(i+1));if(i==0){$(this).show()}})})
$(".bradseg-select-list li").click(function(){$(this).toggleClass("selected")})
if(($(".btn-prev .bradseg-btn:disabled"))&&!($(".btn-prev .bradseg-btn").parent("pre"))){$(".btn-prev .bradseg-btn:disabled").parent().find("span").css("color","#a5aab6")}
if(($(".btn-next .bradseg-btn:disabled"))&&!($(".btn-next .bradseg-btn").parent("pre"))){$(".btn-next .bradseg-btn:disabled").parent().find("span").css("color","#a5aab6")}
$('.datepicker').datepicker({format:"dd/mm/yyyy",todayHighlight:!0}).on('changeDate',function(ev){$(this).datepicker('hide')});var validate={onComplete:function(val,event,thisField){thisField.parent().removeClass('errorTextBox')},onKeyPress:function(val,event,thisField,options){thisField.parent().addClass('errorTextBox')},onChange:function(val,event,thisField){},onInvalid:function(val,event,thisField,invalid,options){}};$('.mask-altura').mask('0,00')
$('.mask-semcharespecial').mask('A',{'translation':{'A':{pattern:/[a-zA-Z0-9]/,recursive:!0}}});$('.mask-bin').mask('0000 00');$('.mask-cep').mask('00.000-000',validate);$('.mask-cnpj').mask('00.000.000/0000-00',validate);$('.mask-cpf').mask('000.000.000-00',validate);$('.mask-cartao').mask('0000 0000 0000 0000',validate);$('.mask-data').mask('00/00/0000',validate);$('.mask-diaMes').mask('00');$('.mask-ano').mask('0000');$('.mask-dinheiro').mask('000.000.000.000.000,00',{reverse:!0});$('.mask-numero').mask('#');$('.mask-porcentagem').mask('000%');var pesoBehaviour=function(val){return val.replace(/\D/g,'').length===5?'000,00':val.replace(/\D/g,'').length===4?'00,009':'0,0099'},pesoOption={onKeyPress:function(val,e,thisField,options){thisField.mask(pesoBehaviour.apply({},arguments),pesoOption)}};$('.mask-peso').mask(pesoBehaviour,pesoOption);$('.mask-placacarro').mask('AAA-0000',validate);var BRMaskBehavior=function(val){return val.replace(/\D/g,'').length===11?'(00) 00000-0000':'(00) 0000-00009'},brOptions={onKeyPress:function(val,e,thisField,options){thisField.mask(BRMaskBehavior.apply({},arguments),brOptions)}};$('.mask-ddd').mask('00');$('.mask-dddtelefone').mask(BRMaskBehavior,brOptions);var BRMaskOnlyPhoneBehavior=function(val){return val.replace(/\D/g,'').length===9?'00000-0000':'0000-00009'},brOnlyPhoneOptions={onKeyPress:function(val,e,thisField,options){thisField.mask(BRMaskOnlyPhoneBehavior.apply({},arguments),brOnlyPhoneOptions)}};$('.mask-telefone').mask(BRMaskOnlyPhoneBehavior,brOnlyPhoneOptions);$('[data-toggle="tooltip"]').tooltip();var smallCount=0;var tallCount=0;var fontCount=0;var colorBlindCount=0;$(".btn_smallFont").click(function(){if(fontCount>0){fontCount=fontCount-1;$('body').css({'font-size':(10+fontCount-0.5)+'px'});$('.dropdownProdutos').css({'font-size':(10+fontCount-0.5)+'px'});$('.dropdownAtendimento').css({'font-size':(10+fontCount-0.5)+'px'})}});$(".btn_tallFont").click(function(){if(fontCount<3){fontCount=fontCount+1;$('body').css({'font-size':(10+fontCount-0.5)+'px'});$('.dropdownProdutos').css({'font-size':(10+fontCount-0.5)+'px'});$('.dropdownAtendimento').css({'font-size':(10+fontCount-0.5)+'px'})}});$(".btn_colorBlind").click(function(){var blackwhite_href=$("#bradsegBlackWhite").attr("data-href");var highcontrast_href=$("#bradsegHighContrast").attr("data-href");if(colorBlindCount<2){if(colorBlindCount==0){document.getElementById('bradsegBlackWhite').href=blackwhite_href;document.getElementById('bradsegHighContrast').href='';$(".logo-bradseg").each(function(){if($(this).is("img")){var bradseg_logo_default=$(this).attr('src');var bradseg_logo_grayscale=$(this).attr('data-grayscale');var bradseg_logo_highcontrast=$(this).attr('data-highcontrast');$(this).attr("src",bradseg_logo_grayscale);$(this).attr("data-grayscale",bradseg_logo_default)}})
colorBlindCount=1}else if(colorBlindCount==1){document.getElementById('bradsegHighContrast').href=highcontrast_href;document.getElementById('bradsegBlackWhite').href='';$(".logo-bradseg").each(function(){if($(this).is("img")){var bradseg_logo_default=$(this).attr('src');var bradseg_logo_grayscale=$(this).attr('data-grayscale');var bradseg_logo_highcontrast=$(this).attr('data-highcontrast');$(this).attr("src",bradseg_logo_highcontrast);$(this).attr("data-grayscale",bradseg_logo_default);$(this).attr("data-highcontrast",bradseg_logo_grayscale)}})
colorBlindCount=2}}else{document.getElementById('bradsegBlackWhite').href='';document.getElementById('bradsegHighContrast').href='';$(".logo-bradseg").each(function(){if($(this).is("img")){var bradseg_logo_default=$(this).attr('src');var bradseg_logo_grayscale=$(this).attr('data-grayscale');var bradseg_logo_highcontrast=$(this).attr('data-highcontrast');$(this).attr("src",bradseg_logo_highcontrast);$(this).attr("data-highcontrast",bradseg_logo_default)}})
colorBlindCount=0}})})
$(document).scroll(function(){navAnchorFixed()});$(window).resize(function(){setBannerHeight();navAnchorBehaviour();navAnchorFixed()})
function navAnchorFixed(){if($(window).width()>769){if(!($("body").hasClass("styleguide"))){var y=$(this).scrollTop();var nav_ancora_position=$("#nav-anchor").offset().top;var nav_main_position=$(".nav-main-menu").offset().top+70;if(y>nav_ancora_position){$(".nav-anchor").addClass("fixed")}else{$(".nav-anchor").removeClass("fixed")}
if(y>nav_main_position){$(".nav-anchor .logo-bradseg").fadeIn()}else{$(".nav-anchor .logo-bradseg").fadeOut()}}}else{$(".nav-anchor").removeClass("fixed")}}
function navAnchorBehaviour(){var anchor_size=$(".nav-anchor .content").width()-40;var anchor_logo_size=$(".nav-anchor .content .logo-bradseg").width()+32;var anchor_vertical_size=$(".nav-anchor .item-vertical").width()+34;var anchor_session_size=$(".nav-anchor .section-all-items").width()+32;var total_elem_size;total_elem_size=anchor_logo_size+anchor_vertical_size+anchor_session_size;if(anchor_size>total_elem_size){$(".nav-anchor .content .section-all-items").css("display","inline-block")
$(".nav-anchor .content>.dropdown").css("display","none")}else{$(".nav-anchor .content .section-all-items").css("display","none")
$(".nav-anchor .content>.dropdown").css("display","inline-block")}}
function setBannerHeight(){window_height=$(window).height();$(".bradseg-img-banner").height(window_height-68)}
